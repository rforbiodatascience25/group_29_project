...
data_analysis5  <- data_aug |>
  ...
  mutate(is_significant = case_when(
    p.value[term != "(Intercept)"] < 0.05 ~ "yes",
    p.value[term != "(Intercept)"] >= 0.05 ~ "no")) |>
  mutate(
...
p1 <- data_analysis5 |>
  CD_order() |>
  plot_CD4vsCD8(pair_filter = "SP1ap",
                legend_position = "none")

p2 <- data_analysis5 |>
  CD_order() |>
  plot_CD4vsCD8(pair_filter = "SP1am",
                legend_position = "none")

p3 <- data_analysis5 |>
  CD_order() |>
  plot_CD4vsCD8(pair_filter = "naive",
                legend_position = "none")
...
p5 <- data_analysis5 |>
  CD_order() |>
  plot_CD4vsCD8(pair_filter = "EM",
                legend_position = "none")

p6 <- data_analysis5 |>
  CD_order() |>
  plot_CD4vsCD8(pair_filter = "TEMRA",
...
```{r}
#| label: CDs_overview_9

final_plot <- data_analysis5 |>
  CD_order() |>
  group_by(CD)|>
  filter(any(is_significant == "yes")) |>
  ungroup() |>
  group_by(CD, pair) |>
  mutate(highest = if_else(
    estimate[str_detect(cell_type, "CD8")] >
    estimate[str_detect(cell_type, "CD4")], 
    "CD8 T cells",
    "CD4 T cells")) |>
  mutate(pair = factor(x = pair, 
                       levels = c("TEMRA",
                                  "EM",
                                  "CM",
                                  "naive",
  ...
  geom_tile(aes(fill = is_significant),
            alpha = 0.4,
            color = "white") +
  geom_point(aes(shape = highest,
                 color = highest,),
             size = 2) +
  scale_fill_manual(values = c("no" = "#fa2840",
                                "yes" = "#29cf8f")) +
  scale_color_manual(values = c("CD4 T cells" = "black",
                               "CD8 T cells" = "#FFD700")) +
  scale_shape_manual(values = c("CD4 T cells" = 16, 
                                "CD8 T cells" = 15)) +
  ...
  theme(axis.text.x = element_text(angle = 90, 
        ...
        legend.box = "vertical") +
  labs(title = "Significant CDs for each pair",
       fill = "Is significant",
       shape = "Highest log(MedQb)",
       color = "Highest log(MedQb)",
       x = "CD",
       y = "Pair")
...
save_plot(plot = sig_CD_count,
          filename = "09_key_plot_1.png")
  
save_plot(plot = final_plot,
          filename = "09_key_plot_2.png",
          width = 10,
          height = 5)
```
