...
The CD markers that generally show the biggest difference are as expected the lineage-specific markers `CD4` and `CD8`, which are significant for all the pairs.

Below are the number of significant CDs for each pair calculated and visualized.

```{r}

sig_CD_count <- CD4_vs_CD8 |>
  group_by(pair) |>
  summarise(count_CD = n_distinct(CD)) |>
  mutate(pair = fct_rev(fct_reorder(pair, count_CD))) |>
  ggplot(aes(x = pair,
             y = count_CD)) +
  geom_col(fill = "hotpink") + 
  theme_bw(base_size = 14) +
  labs(title = "Count of significant CDs for each pair",
       x = "Pair",
       y = "Count")

```

# Plot export and document rendering

As the figure above is too big for the presentation only the plot for the naive T cells are saved.
...
save_plot(plot = final_plot,
          filename = "09_analysis_5_plot.png",
          width = 7,
          height = 9)
```
