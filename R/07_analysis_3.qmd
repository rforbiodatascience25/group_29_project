...
## Correlation of MedQb to CVQb and PEpos

The purpose of these plots is to examine and compare the fluorescence signals across key immune cell types and tissues.
...
p1 <- data_aug |>
  asc_CDs_boxplot(lineage_filter = "B cells",
                  hierarchy_filter = 3,
                  tissue_filter = "blood",
                  measure = MedQb)
...
p9 <- data_aug |>
  plot_scatter(params = data_params,
               measure = CVQb,
               lineage_filter = "Thymocytes",
               tissue_filter = "thymus",
               color = "blue")
...
final_plot <- (p1 + p2) / (p3 + p4) / (p5 + p6) / (p7 + p8) / (p9 + p10) +
  plot_annotation(
    title = "Correlation of the intensity to variation and positivity",
