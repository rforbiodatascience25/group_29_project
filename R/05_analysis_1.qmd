...
PCA_MedQb <- PCA_aug |>
  select(tissue, cell_type, lineage, starts_with("MedQb"))

PCA_PEpos <- PCA_aug |>
  select(tissue, cell_type, lineage, starts_with("PEpos"))
```

```{r}
#| label: create_PCA_fit_5

PCA_fit_PEpos <- PCA_PEpos |> 
  select(where(is.numeric)) |>
  prcomp(scale = TRUE)

PCA_fit_MedQb <- PCA_MedQb |> 
  select(where(is.numeric)) |>
  prcomp(scale = TRUE)
```
...
```{r}
scatter_plot_MedQB <- PCA_plot(PCA_fit_MedQb,PCA_aug, "PCA_MedQb")
```

```{r}
scatter_plot_PEpos <- PCA_plot(PCA_fit_PEpos,PCA_aug, "PCA_PEpos")
```
