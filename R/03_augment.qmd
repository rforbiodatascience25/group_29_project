...
cd_models <- data_sub |>
  group_by(tissue, CD) |>  # Group by CD marker
...
columns <- c("CD", "tissue", "cell_type_compared", "p.value", "estimate", "conf.low", "conf.high")
...
cd_results <- cd_models |>
  ...
  select(columns) |>
  ungroup() |>
  mutate(                   # Adjust p-values for multiple testing
    is_significant = case_when(p.value < 0.05 ~ "yes",
                               p.value >= 0.05 ~ "no")  # Mark significant differences
  )
